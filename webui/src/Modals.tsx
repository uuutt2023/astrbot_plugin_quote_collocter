...
import {
  Modal, Button, Form, Input, InputNumber, Radio, Slider, Space, message, Upload, App, Alert, Spin,
} from "antd";
import {
  ...
  useEffect(() => {
    if (q.data) {
      form.setFieldsValue({
        ...
        poke_probability: typeof q.data.poke_probability === "number" ? q.data.poke_probability : 0.85,
      });
    }
  }, [q.data, form]);
    ...
    >
      {q.isLoading || !q.data ? (
        ...
        <Form form={form} layout="vertical">
          ...
          <Form.Item
            name="poke_probability"
            label={t(ctx, "settings.pokeProbability")}
            extra={t(ctx, "settings.pokeProbabilityHelp")}
          >
            <Slider
              min={0}
              max={1}
              step={0.05}
              marks={{ 0: "0", 0.5: "0.5", 0.85: "0.85", 1: "1" }}
            />
          </Form.Item>
        </Form>
      )}
